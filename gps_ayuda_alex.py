...
def generar_grafos():  
    ...
    # Limpiamos y añadimos informacion extra a direcciones
    ...
    for fila in cruces.itertuples():
        ...
        # Dado que las calles pueden seguir distinto orden entre pares e impares, escogemos dentro del df temporal los numeros
        # pares o impares (el que tenga mas numeros) para que el orden de los numeros de calle sea el mismo en ambos casos
        ...
        if (fila[0] == len(cruces) - 1) or (calle_act != calle):
            ...
            if numero_calle in dict_vertices:
                # Vamos a ver a cuanto esta del otro vertice con el mismo numero, y en caso de estar lo suficientemente lejos, le asignamos
                # otro numero
                ...
                if dist > 1000:
                    numero_calle = np.random.randint(0, 100) / 1000
                dict_vertices[numero_calle] = vertices[coords]
            else:
                dict_vertices[numero_calle] = vertices[coords]
